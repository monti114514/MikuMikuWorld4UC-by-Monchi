...
namespace MikuMikuWorld
{
	namespace
	{
		...
		Color previewDefaultTint{ 1.f, 1.f, 1.f, 1.f };

		DirectX::XMFLOAT4 previewToFloat4(const Color& color, float alphaScale = 1.0f)
		{
			return { color.r, color.g, color.b, color.a * alphaScale };
		}

		std::array<DirectX::XMFLOAT4, 4> previewGetUV(float left, float right, float top, float bottom)
		{
			return {{
				{ right, top, 0.f, 1.f },
		...
		bool isLayerHideNotesActive(const Score& score, int layer, int tick)
		{
		...
		int getConnectorLayerZOffset(HoldStepLayer layer)
		{
			switch (layer)
			{
		...
		std::vector<double> getCurrentLayerScaledTimes(const ScoreContext& context)
		{
	...
	void ScorePreviewWindow::drawLines(const ScoreContext& context, Renderer* renderer)
	{
		...
		for (auto& line : drawData)
		{
			...
			renderer->pushQuad(vPos, uv, DirectX::XMMatrixIdentity(), previewToFloat4(previewDefaultTint), (int)texture.getID(), zIndex);
		}
	}
	...
	void ScorePreviewWindow::drawHoldTicks(const ScoreContext& context, Renderer* renderer)
	{
		...
		for (auto& tick : context.scorePreviewDrawData.drawingHoldTicks)
		{
			...
			auto uv = previewGetUV(sprite.getX() / texW,
			                       (sprite.getX() + sprite.getWidth()) / texW,
			                       sprite.getY() / texH,
			                       (sprite.getY() + sprite.getHeight()) / texH);

			renderer->pushQuad(vPos, uv, DirectX::XMMatrixScaling(y, y, 1.f),
			                   previewToFloat4(previewDefaultTint), (int)texture.getID(),
		...
		for (auto& segment : drawData.drawingHoldSegments)
		{
			...
			for (int i = 0; i < steps; i++)
			{
				...
				if (segment.isGuide)
				{
					...
					vertexColors = {{
						previewToFloat4(previewDefaultTint, startAlpha),
						previewToFloat4(previewDefaultTint, endAlpha),
						previewToFloat4(previewDefaultTint, endAlpha),
						previewToFloat4(previewDefaultTint, startAlpha)
					}};
				...
				float texW = (float)texture.getWidth();
				float texH = (float)texture.getHeight();
				auto uv = previewGetUV(spr_x1 / texW, spr_x2 / texW, spr_y1 / texH, spr_y2 / texH);
				...
				if (config.pvHoldAnimation && isHoldActivated && !segment.isGuide && isArrayIndexInBounds(sprIndex - 1, texture.sprites))
				{
					...
					renderer->pushQuad(vPos, uv, model, previewToFloat4(previewDefaultTint, baseAlpha * normalAplha), (int)texture.getID(), zIndex);
					auto uvActive = previewGetUV(spr_x1 / texW, spr_x2 / texW, (spr_y1 + norm2ActiveOffset) / texH, (spr_y2 + norm2ActiveOffset) / texH);
					renderer->pushQuad(vPos, uvActive, model, previewToFloat4(previewDefaultTint, baseAlpha * (1.f - normalAplha)), (int)texture.getID(), zIndex);
				}
				...
				{
					renderer->pushQuad(vPos, uv, model, previewToFloat4(previewDefaultTint, baseAlpha), (int)texture.getID(), zIndex);
				}
	...
	void ScorePreviewWindow::drawNoteBase(Renderer* renderer, const Note& note, float noteLeft, float noteRight, float y, float zScalar)
	{
		...
		if (geomWidth > 0.0f)
		{
			...
			uv = previewGetUV(midUvLeft / texW, midUvRight / texW, sprite.getY() / texH, (sprite.getY() + sprite.getHeight()) / texH);
			renderer->pushQuad(vPos, uv, model, previewToFloat4(previewDefaultTint), (int)texture.getID(), zIndex);
		}

		vPos = lTransform.apply(Engine::perspectiveQuadvPos(noteLeft, noteLeft + 0.25f, noteTop, noteBottom));
		uv = previewGetUV((sprite.getX() + NOTE_SIDE_PAD) / texW,
		                  (sprite.getX() + NOTE_SIDE_WIDTH) / texW,
		                  sprite.getY() / texH,
		                  (sprite.getY() + sprite.getHeight()) / texH);
		renderer->pushQuad(vPos, uv, model, previewToFloat4(previewDefaultTint), (int)texture.getID(), zIndex);

		vPos = rTransform.apply(Engine::perspectiveQuadvPos(noteRight - 0.3f, noteRight, noteTop, noteBottom));
		uv = previewGetUV((sprite.getX() + sprite.getWidth() - NOTE_SIDE_WIDTH) / texW,
	...
	void ScorePreviewWindow::drawTraceDiamond(Renderer* renderer, const Note& note, float noteLeft, float noteRight, float y)
	{
		...
		float texW = (float)texture.getWidth();
		float texH = (float)texture.getHeight();
		auto uv = previewGetUV(frictionSpr.getX() / texW,
		                       (frictionSpr.getX() + frictionSpr.getWidth()) / texW,
		                       frictionSpr.getY() / texH,
		                       (frictionSpr.getY() + frictionSpr.getHeight()) / texH);
	...
	void ScorePreviewWindow::drawFlickArrow(Renderer* renderer, const Note& note, float y, double time)
	{
		...
		float texW = (float)texture.getWidth();
		float texH = (float)texture.getHeight();
		auto uv = previewGetUV(arrowSprite.getX() / texW,
		                       (arrowSprite.getX() + arrowSprite.getWidth()) / texW,
		                       arrowSprite.getY() / texH,
		                       (arrowSprite.getY() + arrowSprite.getHeight()) / texH);
		...
		if (config.pvFlickAnimation)
		{
			...
			renderer->pushQuad(vPos, uv, model, previewToFloat4(previewDefaultTint, 1.f - cubicEaseIn(t)), (int)texture.getID(), zIndex);
		}
		else
		{
			auto model = DirectX::XMMatrixScaling(y, y, 1.f);
